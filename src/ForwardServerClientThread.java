...
public class ForwardServerClientThread extends Thread
{
    ...
    public ForwardServerClientThread(Socket aClientSocket, String serverhost, int serverport, SessionEncrypter encrypter) throws IOException
    {
        mClientSocket = aClientSocket;
        mServerPort = serverport;
        mServerHost = serverhost;
    ...
    public ForwardServerClientThread(ServerSocket listensocket, String serverhost, int serverport, SessionDecrypter decrypter) throws IOException
    {
        mListenSocket = listensocket;
        //mServerHost =  listensocket.getInetAddress().getHostAddress();
        mServerPort = serverport;
    ...
    public void run()
    {
        try {
           ...
           //If this session has encryption, set the output streams.
           if (decrypter != null) clientIn = decrypter.openCipherInputStream(clientIn);
           if (encrypter != null) serverOut = encrypter.openCipherOutputStream(serverOut);
